...
export function DashboardSidebar() {
  ...
  const sidebarClasses = isDrawerOpen
    ? "fixed inset-y-0 left-0 z-50 flex w-80 max-w-[88vw] translate-x-0 rounded-none transition-transform lg:static lg:w-auto lg:max-w-none lg:rounded-4xl"
    : "hidden lg:flex lg:static lg:w-auto lg:max-w-none lg:rounded-4xl";

  return (
    <aside
      className={`${sidebarClasses} digital-panel-strong flex-col p-5 text-white lg:sticky lg:top-8 lg:h-[calc(100vh-4rem)] bg-[rgba(8,10,13,0.9)]`}
    ...
    >
      {isDrawerOpen && (
        <button
          ...
          className="fixed inset-0 -z-10 cursor-default bg-black/45 backdrop-blur-[1px] lg:hidden"
        />
      )}
      {/* mobile close button */}
